#!/usr/bin/env python
"""
Backfill historical odds for AFL events into the Bronze layer.

Historical odds capture the entire time series of price movements from
the market opening through to match close. This script reads event
URLs for past rounds and obtains historical markets either via
bookmaker APIs or archived webpages. Rows are written to
``bronze/odds/history`` with a ``source_kind`` column set to
``'historical'``.

As with live odds, records are deduplicated based on a hash of key
fields. Duplicate rows are skipped unless the ``--force`` flag is
provided.
"""
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if output_root.exists() and not args.force:
        try:
            con.execute(f"CREATE VIEW odds_hist AS SELECT * FROM read_parquet('{output_root}/**/*.parquet')")
    ...
    new_rows = []
    # Generate dummy historical odds: two timepoints 48 and 24 hours before kickoff
    for _, row in event_df.iterrows():
        bookmaker = row["bookmaker"]
        event_url = row["event_url"]
        # We'll pretend kickoff is scheduled_time_utc plus 48 hours from now for demonstration
        kickoff = utc_now() + timedelta(days=2)
        timestamps = [kickoff - timedelta(hours=48), kickoff - timedelta(hours=24)]
        for t in timestamps:
            record = {
                "captured_at_utc": t,
                "bookmaker": bookmaker,
