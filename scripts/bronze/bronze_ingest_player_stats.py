...
def _find_stat_table(soup: BeautifulSoup, section_title: str) -> Optional[pd.DataFrame]:
    ...
    """
    # 1) Find the visible text node for the title
    title_node = soup.find(string=re.compile(rf"^\s*{re.escape(section_title)}\s*$", re.I))
    if not title_node:
        return None

    # 2) Walk forward until we hit a valid GBG table
...
def _extract_opponent_row(df: pd.DataFrame) -> Dict[str, str]:
    """
    Get mapping round_label -> opponent 2-3 letter code from 'Opponent' row (if present).
    """
    df = df.copy()
    df.columns = _flatten_columns(df.columns)
...
def parse_team_season(team: str, slug: str, year: int, session: Optional[requests.Session] = None,
    ...
    desired = ["season", "team", "player", "round_label", "round_num", "is_finals",
               "opponent_code", "DI", "KI", "MK", "HB","GL","BH","HO","TK","RB","I5","CL","CG","FF","FA","CP","UP","CM","MI","OP","GA","%P"]
...
def write_partitioned(df: pd.DataFrame, out_root: str, season: int, minimize: bool = False, verbose: bool = False):
    ...
    write_df = df.copy()
    if minimize:
        keep = ["season","team","player","round_label","round_num","is_finals","opponent_code","DI","KI", "MK", "HB","GL","BH","HO","TK","RB","I5","CL","CG","FF","FA","CP","UP","CM","MI","OP","GA","%P"]
...
def main():
    ap = argparse.ArgumentParser(description="Ingest AFLTables team Game-by-Game (DI/KI/MK/HB) into Bronze.")
